...
A random search algorithm is used to select a tuned mtry value.  The mtry values is the number of randomly drawn candidate variables.  We expect mtry to be an acronym but we do not find any confirmation in the literature.
...
The visualization above shows that random search found the model's accuracy to be highest at mtry = 2. 
...
The grid search above was conducted to compare with the previous random search for an optimum mtry vale.  The grid search confirms a peak accuracy at mtry = 2.  The randomForest model is tested with this optimized mtry value below.
