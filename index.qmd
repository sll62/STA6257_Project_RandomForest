...
scaleable classifier. In one study, researchers create a Random Forest
...
In another experiment, researchers develope a random forest regression
...
Pakistan and consists of the medical records of 299 patients; some who
...
The dataset is checked for any null values, and none are found. Null
...
The target variable (DEATH_EVENT) is examined for the balance
...
heart_df_noBinary <- select(heart_df, -c(anaemia, diabetes, high_blood_pressure, sex, smoking, DEATH_EVENT))

cor_matrix <- cor(heart_df_noBinary)
...
Cross validation for model without sodium: 
```{r}
# No Sodium model cross validation

heart_dv_noSerumSodium <- select(heart_df, -c(serum_sodium))
...
heart_train_noSodium <- heart_dv_noSerumSodium[samp, ]
heart_test_noSodium <- heart_dv_noSerumSodium[-samp, ]
# Set seed for reproducibility
set.seed(123)
# Train a random forest model
noSodium_forest <- train(

        # Formula. We are using all variables to predict Species
        DEATH_EVENT~.,

        # Source of data; remove the Species variable
        data=heart_train_noSodium,
...
noSodium_forest$finalModel
```
Cross validation for model without sodium or age:
```{r}
# No Sodium or Age model cross validation

heart_dv_noSodiumAge <- select(heart_df, -c(serum_sodium, age))
...
# Split into training and testing 
set.seed(123)
samp <- sample(nrow(heart_dv_noSodiumAge), 0.8 * nrow(heart_dv_noSodiumAge))

heart_train_noSodiumAge <- heart_dv_noSodiumAge[samp, ]
heart_test_noSodiumAge <- heart_dv_noSodiumAge[-samp, ]
# Set seed for reproducibility
set.seed(123)
# Train a random forest model
noSodiumAge_forest <- train(

        # Formula. We are using all variables to predict Species
        DEATH_EVENT~.,

        # Source of data
        data=heart_train_noSodiumAge,
...
# 
# ## Print out the details about the model
noSodiumAge_forest$finalModel
```
Cross validation for model without sodium, age, or ejection fraction:
```{r}
# No Sodium, Age, or Ejection Fraction model cross validation

heart_dv_noSodiumAgeEj <- select(heart_df, -c(serum_sodium, age, ejection_fraction))
...
# Split into training and testing 
set.seed(123)
samp <- sample(nrow(heart_dv_noSodiumAgeEj), 0.8 * nrow(heart_dv_noSodiumAgeEj))

heart_train_noSodiumAgeEj <- heart_dv_noSodiumAgeEj[samp, ]
heart_test_noSodiumAgeEj <- heart_dv_noSodiumAgeEj[-samp, ]
# Set seed for reproducibility
set.seed(123)
# Train a random forest model
noSodiumAgeEj_forest <- train(

        # Formula. We are using all variables to predict Species
        DEATH_EVENT~.,

        # Source of data
        data=heart_train_noSodiumAgeEj,
...
# 
# ## Print out the details about the model
noSodiumAgeEj_forest$finalModel
```
Cross validation for model without sodium, age, ejection fraction, or platelets:
```{r}
# No Sodium, Age, Ejection Fraction, or Platelets model cross validation

heart_dv_noSodiumAgeEjPlt <- select(heart_df, -c(serum_sodium, age, ejection_fraction, platelets))
...
# Split into training and testing 
set.seed(123)
samp <- sample(nrow(heart_dv_noSodiumAgeEjPlt), 0.8 * nrow(heart_dv_noSodiumAgeEjPlt))

heart_train_noSodiumAgeEjPlt <- heart_dv_noSodiumAgeEjPlt[samp, ]
heart_test_noSodiumAgeEjPlt <- heart_dv_noSodiumAgeEjPlt[-samp, ]
# Set seed for reproducibility
set.seed(123)
# Train a random forest model
noSodiumAgeEjPlt_forest <- train(

        # Formula. We are using all variables to predict Species
        DEATH_EVENT~.,

        # Source of data
        data=heart_train_noSodiumAgeEjPlt,
...
# 
# ## Print out the details about the model
noSodiumAgeEjPlt_forest$finalModel
```
Cross validation for model without sodium, age, ejection fraction, platelets, or serum creatinine:
```{r}
# No Sodium, Age, Ejection Fraction, Platelets, or Serum Creatinine model cross validation

heart_dv_noSodiumAgeEjPltSC <- select(heart_df, -c(serum_sodium, age, ejection_fraction, platelets, serum_creatinine))
...
# Split into training and testing 
set.seed(123)
samp <- sample(nrow(heart_dv_noSodiumAgeEjPltSC), 0.8 * nrow(heart_dv_noSodiumAgeEjPltSC))

heart_train_noSodiumAgeEjPltSC <- heart_dv_noSodiumAgeEjPltSC[samp, ]
heart_test_noSodiumAgeEjPltSC <- heart_dv_noSodiumAgeEjPltSC[-samp, ]
# Set seed for reproducibility
set.seed(123)
# Train a random forest model
noSodiumAgeEjPltSC_forest <- train(

        # Formula. We are using all variables to predict Species
        DEATH_EVENT~.,

        # Source of data
        data=heart_train_noSodiumAgeEjPltSC,
...
# 
# ## Print out the details about the model
noSodiumAgeEjPltSC_forest$finalModel
```
-->
...
at a location that is influenced by their accuracy (left plot) and Gini Coefficient (right plot).
...
influence.  Notice that there is not always a correlation between influence on accuracy and Gini Index.  For example, <em>platelets</em> have a somewhat high Gini Index of about 10-12, but have a small impact on accuracy.
...
waits for a follow-up visit, the more inaccurate the classification becomes.
...
Decrease Gini Index plot.  We note that the information provided by these plots is slightly different than the results provided by the Gini Index table seen previously.  The Gini Index table suggests smoking, high blood pressure, and creatine phosphokinase are the top three variables for classification of heart failure.  We recognize that all sources of analysis must be double-checked and weighed before achieving a final conclusion.
...
variables like serum_creatinine and creatinine_phosphokinase intersect with no
...
causing another variable.  Furthermore, we also recognize that correlation does not always mean causation, so we are
...
When looking at the default model (Model 1) we received an OOB error rate of
13.39%, which translates to a training accuracy of 86.61%. The testing
accuracy is 0.8167 (81.67%). The precision value is 70.59% which means
...
The precision value comes to 0.7500 which indicates that the observations
...
we do not fully understand how it classifies data or which variables are
...
classification process.  A review of the Gini Index table suggests <em>smoking</em>, <em>high blood pressure</em>, and <em>creatine phosphokinase</em> are also strong contributors.
