...
The dataset was checked for any null values.
```{r}
#{r, echo=FALSE}
...
The ROC (receiver operating characteristic) curve above provides a graphical representation of the true positive rate along the Y-axis vs. the false positive rate along the X-axis.  

The classification accuracy with the default hyperparameters for the randomForest package comes to 81.67%.

### Running the Random Forest Model - Tuning Attempt #1

A random search algorithm was used to select a tuned mtry value.
```{r}
ncol(heart_train)
```
...
The random search found the model's accuracy to be highest at mtry = 2. 
A grid search was then conducted to see if another mtry may provide increased accuracy.
...
The grid search shows a peak accuracy at mtry = 2 so this appears to be an mtry value that will improve the accuracy of the model. The randomForest model that was created earlier can now be tested with this new mtry.
...
After changing the mtry hyperparameter to 5, the OOB estimate of error rate decreased slightly to 16.74%. The "positive" classification error improved, coming to about 28.0%. The "negative" classification error increased slightly at a new value of about 11.6%.
