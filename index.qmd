...
The results suggest that, of the ntree values tested, the model's accuracy would be highest with an ntree value of 1300 as shown in the dotplot. This new ntree values is tested, along with the tuned mtry value of 5, using the randomForest model from before.
...
The OOB value decreased, in relation to the previous models, to 15.9% while the classification error rate for the positive class slightly increased to 29.3% and the negative class classification error rate decrease to 9.76%.
...
The accuracy and area under the receiver operating characteristic curve (AUC-ROC) are higher for the model produced using the default hyperparameters (88.3% and 0.933, respectively) compared to the "tuned" model (86.7% and 0.910, respectively).
