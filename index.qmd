...
The above table needs to be explained.  What are 0, 1, 40, and 20?
...
The above table needs to be explained.  What is "2" and what is "1".
...
The classification accuracy of the model is calculated to be 88.33%.  This is consistent with previous calculations that provide an accuracy measurement as part of its output (e.g., Confusion and Matrix Statistics).
...
```
[Is the graph correct?  Why does the X-axis start at 1 and go to 0?]
...
Without a correct plot, I cannot comment further.
...
Above is an alternate implementation of the Random Forest algorithm which produces a classification accuracy of 83.5% and a Cohen's kappa of 0.607.  Kappa is used to test <em>interrater reliability</em> which "represents the extent to which the data collected in the study are correct representations of the variables measured" [@mchugh2012].  A kappa score can range from -1 to +1 where 1 is the highest accuracy.  Kappa is regarded as a stronger indicator of accuracy over calculating the number of agreement score divided by the total number of scores.

which is calculated by the observed and expected accuracy.
...
How is the above different than the default approach?
